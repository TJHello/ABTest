 ...
 * 时间:2020/2/24 10:21
 */
class ABTestOld private constructor(private val context: Context) {
    ...
    companion object{
        ...
        private var abConfigList : MutableList<ABConfigOld> = mutableListOf()
        ...
        private lateinit var abTest: ABTestOld

        private lateinit var tools : Tools

        @JvmStatic
        fun init(context: Context,isNew:Boolean):ABTestOld{
            if(!::abTest.isInitialized){
                abTest = ABTestOld(context)
                tools = Tools(context)
        ...
        private fun canABTest(abConfig: ABConfigOld):Boolean{
    ...
    fun addTest(context: Context,config: ABConfigOld):ABTestOld{
    ...
    fun fixedValue(name:String,value:String):ABTestOld{
        fixedValue[name] = value
        return this
    }

    fun startTimeTack():ABTestOld{
